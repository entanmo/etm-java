...
import com.etm.sdk.codec.Base58;
...
import com.etm.sdk.security.ripemd.RipeMD160;
...
public class DefaultSecurityStrategy implements SecurityStrategy{
    ...
    private static final EtmSDKConfig config = EtmSDKConfig.getInstance();
    ...
    public String getBase58Address(String publicKey) throws SecurityException{
        try{
            ...
            return EtmConst.BASE58_ADDRESS_PREFIX + Encoding.base58(buffer);
        }
        catch (Exception ex){
    ...
    public boolean isValidBase58Address(String address){
        if (null == address || address.length() < EtmConst.MIN_BASE58_ADDRESS_LEN){
            return false;
        }

        if (address.charAt(0) != EtmConst.BASE58_ADDRESS_PREFIX){
            return false;
        }
